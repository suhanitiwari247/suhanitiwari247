<h1 align="center">Hi 👋, I'm Suhani Tiwari</h1>
<h3 align="center">DSA Proficient | Java Enthusiast | Backend Developer | Creative Frontend Learner</h3>

---

### 👩‍💻 About Me

- 🎓 Computer Science student from Bhopal, Madhya Pradesh  
- 💻 Strong in **DSA**, **Java**, and **C++**
- 🌐 Learning and building projects with **ReactJS**, **HTML/CSS**, **JavaScript**, **NestJS**, **Next.js**  
- 🛠️ Exploring **Backend Development**, **Cloud Systems**, and **System Design**  
- 🧠 Passionate about solving real-world problems through technology  
- 🏆 Active participant in **coding contests** and **hackathons**  
- 📫 Reach me at: **suhanitiwari247@gmail.com**

---

### 💡 Skills

<p align="center">
  <!-- Languages -->
  <img src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=java&logoColor=white" />
  <img src="https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=c%2B%2B&logoColor=white" />
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" />
  <!-- Frontend -->
  <img src="https://img.shields.io/badge/HTML-E34F26?style=for-the-badge&logo=html5&logoColor=white" />
  <img src="https://img.shields.io/badge/CSS-1572B6?style=for-the-badge&logo=css3&logoColor=white" />
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black" />
  <img src="https://img.shields.io/badge/ReactJS-61DAFB?style=for-the-badge&logo=react&logoColor=black" />
  <img src="https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=nextdotjs&logoColor=white" />
  <!-- Backend -->
  <img src="https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white" />
  <img src="https://img.shields.io/badge/NestJS-E0234E?style=for-the-badge&logo=nestjs&logoColor=white" />
  <img src="https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white" />
  <!-- Databases -->
  <img src="https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white" />
  <img src="https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb&logoColor=white" />
  <img src="https://img.shields.io/badge/DynamoDB-4053D6?style=for-the-badge&logo=amazondynamodb&logoColor=white" />
  <!-- ML & Tools -->
  <img src="https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white" />
  <img src="https://img.shields.io/badge/scikit--learn-F7931E?style=for-the-badge&logo=scikit-learn&logoColor=white" />
  <img src="https://img.shields.io/badge/Jupyter-F37626?style=for-the-badge&logo=jupyter&logoColor=white" />
  <img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black" />
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" />
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
</p>

---

### 💻 Projects

- ☁️ **Cloud Security Analyzer** — *Python, Flask, Azure SDK*  
  Analyzed Azure resources for misconfigurations, automated vulnerability alerts, and simulated Zero Trust cloud security.  

- 🛡️ **PhishGuard – AI-Based Email Phishing Detector** — *Python, scikit-learn, Flask*  
  Built an ML model classifying phishing emails with 94% accuracy; deployed as a web app for real-time detection.  

- 🔐 **ZeroTrust Network Simulator** — *Node.js, React.js, MongoDB, JWT, Docker*  
  Designed microservices enforcing Zero Trust authentication and role-based access; deployed using Docker containers.  

- 📊 **Cloud-Based Attendance Management System** — *React.js, Node.js, AWS Lambda, DynamoDB*  
  Implemented serverless attendance tracking for 500+ users, reducing manual efforts by 70%; integrated facial recognition for automated check-ins.  

---

### 💼 Career Goals

- 🔍 Preparing for **SDE roles** at product-based and service-based companies  
- 💡 Focused on **full stack development**, **cloud computing**, **cybersecurity**, and **ML/AI applications**  
- 🤝 Open to internships, projects, and collaborations  

---

### 📚 Current Focus

- ✅ Java DSA + OOP Mastery  
- ✅ Full Stack (MERN + Next.js + NestJS)  
- ✅ Cloud & Zero Trust Security  
- ✅ ML/AI Deployment & Jupyter Workflow  
- ✅ System Design Basics  
- ✅ Git & GitHub Workflow  

---

### 🌐 Connect with Me

- 💼 [LinkedIn – Suhani Tiwari](https://www.linkedin.com/in/suhani-tiwari-347a57329)  
- 📧 Email: **suhanitiwari247@gmail.com**  
- 💻 GitHub: [github.com/suhanitiwari247](https://github.com/suhanitiwari247)  

---

### 📈 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=suhanitiwari247&show_icons=true&theme=radical" alt="GitHub Stats" />
  <br/>
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=suhanitiwari247&theme=radical" alt="GitHub Streak" />
</p>

---

### ✨ Quote I Live By
> *“Skills get you interviews. Projects and consistency get you hired.”*
